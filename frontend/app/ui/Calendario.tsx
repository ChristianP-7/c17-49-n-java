...
export const Calendario: React.FC<Props> = () => {
  ...
  const [diaSeleccionado, setDiaSeleccinado] = useState<number>(
    new Date().getDate(),
  );
  ...
  const diasEnMes: number = new Date(
    fecha.getFullYear(),
    fecha.getMonth() + 1,
    0,
  ).getDate();
  ...
  return (
    <div className="flex items-center justify-center from-red-100 via-red-300 to-red-500 h-58">
      <div className="md:w-[550px] md:h-[230px] p-4 max-w-[350px] md:max-w-full overflow-hidden rounded-2xl shadow-2xl flex flex-col bg-white">
        ...
        {[...Array(Math.ceil((diasEnMes + primerDiaDelMes) / 7))].map(
          (_, semanaIndex) => (
            <div
              key={semanaIndex}
            ...
            >
              {[...Array(7)].map((_, diaIndex) => {
                ...
                const estiloDia = `px-1 w-14 flex justify-center items-center border rounded-xl hover:border-mlt-600 cursor-pointer ${
                  isSelected
                    ? 'text-white bg-mlt-600 rounded-2xl shadow-md'
                    : ''
                } ${
                ...
                return (
                  <span
                    ...
                    className={estiloDia}
                    onClick={() =>
                      esDiaDelMes && !esDomingo ? seleccionarDia(dia) : null
                    }
                  >
                    {esDiaDelMes ? dia : ''}
                  </span>
